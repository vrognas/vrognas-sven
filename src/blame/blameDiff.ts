 ...
 * previous change (a pegged 2-entry log - the prior revision of the file
 * is rarely revision-1) and open the parallel-fetch diff. When the blamed
...
export async function openBlameRevisionDiff(
  ...
  try {
    revs = await source.log(revision, "1", 2, uri, revision);
  } catch {
  ...
  if (revs.length < 2) {
    ...
    try {
      const content = await source.show(uri, revision);
  ...
  return openDiffCompared(source, uri, revs[1]!.revision, revision);
}
