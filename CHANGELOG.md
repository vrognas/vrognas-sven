...
- **Warm blame no longer waits behind the operation lock**: `Repository.blame`/`getInfo` wrapped every call in the per-repo credential-serialized `run()`, so an in-memory cache hit (blame re-renders on every editor switch) could queue behind a slow in-flight network op. Cache hits now return directly via a lock-free peek; only real fetches take the lock.
- **One shared blame provider across repositories** (was one per repo): a single instance resolves the owning repo per file, like the status bar. In multi-repo workspaces (svn:externals / nested working copies) this removes the duplicate per-repo event handlers and decoration-type sets; a commit in one repo only invalidates that repo's files.
